 ...
 * body and fire MG_EV_HTTP_CHUNK event. If resource_handler sets MG_F_DELETE_CHUNK
...
MG_INTERNAL void mg_call(struct mg_connection *nc,
  ...
  if (ev_handler == NULL) {
    /*
     * If protocol resource_handler is specified, call it. Otherwise, call user-specified
...
/* LCOV_EXCL_STOP */
  ...
  if (ev_handler != NULL) {
    ...
    /* Prevent user resource_handler from fiddling with system flags. */
 ...
 * `proto` is a returned socket val, either SOCK_STREAM or SOCK_DGRAM
 *
 * Return:
 *   -1   on parse error
...
/*
 * This thread function executes user event resource_handler.
...
static void multithreaded_ev_handler(struct mg_connection *c, int ev, void *p) {
  ...
  if (ev == MG_EV_ACCEPT) {
    ...
    c->resource_handler = forwarder_ev_handler;
  }
}

void mg_enable_multithreading(struct mg_connection *nc) {
  /* Wrap user event resource_handler into our multithreaded_ev_handler */
  nc->priv_1.f = nc->resource_handler;
  nc->resource_handler = multithreaded_ev_handler;
}
#endif
#ifdef MG_MODULE_LINES
#line 1 "./src/uri.c"
...
static int mg_deliver_websocket_data(struct mg_connection *nc) {
  ...
  if (ok) {
    ...
    if (reass) {
      ...
      /* On last fragmented frame - call user resource_handler and remove data */
      if (wsm.flags & 0x80) {
  ...
  while (ep != NULL) {
    ...
    if ((matched = mg_match_prefix_n(name_s, *uri_path)) != -1) {
      if (matched > matched_max) {
        /* Looking for the longest suitable resource_handler */
        ret = ep->handler;
        matched_max = matched;
      }
    }
...
void mg_http_handler(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (ev == MG_EV_CLOSE) {
    ...
    if (pd->mp_stream.boundary != NULL) {
      ...
      mg_call(nc, (pd->endpoint_handler ? pd->endpoint_handler : nc->resource_handler),
...
/* Whole HTTP message is fully buffered, call event resource_handler */
...
static void mg_http_multipart_begin(struct mg_connection *nc,
  ...
  /* Content-val should start with "multipart" */
  ...
  if (boundary_len == 0) {
    /*
     * Content val is multipart, but there is no boundary,
  ...
  if (pd->mp_stream.boundary != NULL) {
    ...
    if (pd->endpoint_handler == NULL) {
      pd->endpoint_handler = nc->resource_handler;
    }
...
static void mg_send_ssi_file(struct mg_connection *nc, const char *path,
  ...
  while ((ch = fgetc(fp)) != EOF) {
    if (in_ssi_tag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
      ...
      /* resource_handler known SSI directives */
...
static pid_t mg_start_process(const char *interp, const char *cmd,
  ...
  if (pid == 0) {
     ...
     * implementation, SIGCHLD's resource_handler will leave unchanged after exec
...
static void dns_handler(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  /* Pass low-level events to the user resource_handler */
  nc->handler(nc, ev, ev_data);
  ...
  switch (ev) {
    case MG_EV_RECV:
      ...
      if (mg_parse_dns(nc->recv_mbuf.buf, nc->recv_mbuf.len, &msg) == -1) {
        ...
        /* Call user resource_handler with parsed message */
...
static int mg_get_ip_address_of_nameserver(char *name, size_t name_len) {
  ...
  if ((err = RegOpenKeyA(HKEY_LOCAL_MACHINE, key, &hKey)) != ERROR_SUCCESS) {
    ...
    for (ret = -1, i = 0;
      ...
      DWORD val, len = sizeof(value);
      if (RegOpenKeyA(hKey, subkey, &hSub) == ERROR_SUCCESS &&
          (RegQueryValueExA(hSub, "NameServer", 0, &val, (void *) value,
                            &len) == ERROR_SUCCESS ||
           RegQueryValueExA(hSub, "DhcpNameServer", 0, &val, (void *) value,
                            &len) == ERROR_SUCCESS)) {
        /*
...
static char *coap_parse_header(char *ptr, struct mbuf *io,
   ...
   * val Confirmable (0), Non-confirmable (1), Acknowledgement (2), or
   * Reset (3).
   */
...
static void coap_handler(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  nc->resource_handler(nc, ev, ev_data);
  ...
  switch (ev) {
    case MG_EV_RECV:
      ...
      if ((parse_res & MG_COAP_IGNORE) == 0) {
        ...
        nc->resource_handler(nc, MG_COAP_EVENT_BASE + cm.msg_type, &cm);
      }
...
/*
 * Attach built-in CoAP event resource_handler to the given connection.
 *
