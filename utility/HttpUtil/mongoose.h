...
/*
 * Callback function (event resource_handler) prototype, must be defined by user.
 * Mongoose calls event resource_handler, passing events defined below.
 */
...
struct mg_connection {
  ...
  mg_event_handler_t proto_handler; /* Protocol-specific event resource_handler */
  void *proto_data;                 /* Protocol-specific data */
  void (*proto_data_destructor)(void *proto_data);
  mg_event_handler_t handler; /* Event resource_handler function */
...
/*
 * Create a connection, associate it with the given socket and event resource_handler,
 ...
 *
 * See the `mg_add_sock_opts` structure for a dc of the options.
 */
 ...
 *
 * See the `mg_bind_opts` structure for a dc of the optional
 * parameters.
 *
 ...
 * `[::1]:80`
 *
 * See the `mg_connect_opts` structure for a dc of the optional
 * parameters.
 *
 ...
 * and run event resource_handler in that thread. Thus, if an event hanler is doing
 ...
 * an event resource_handler. This function is supposed to be called from the
 * event resource_handler.
 */
...
/*
 * Attach built-in HTTP event resource_handler to the given connection.
 * User-defined event resource_handler will receive following extra events:
 *
 * - MG_EV_HTTP_REQUEST: HTTP request has arrived. Parsed HTTP request
 *  is passed as
 *   `struct http_message` through the resource_handler's `void *ev_data` pointer.
 ...
 *   resource_handler's `void *ev_data` pointer. `http_message::body` would contain
 *   incomplete, reassembled HTTP body.
 *   It will grow with every new chunk arrived, and
 *   potentially can consume a lot of memory. An event resource_handler may process
 *   the body as chunks are coming, and signal Mongoose to delete processed
 *   body by setting `MG_F_DELETE_CHUNK` in `mg_connection::flags`. When
 *   the last zero chunk is received,
 *   Mongoose sends `MG_EV_HTTP_REPLY` event with
 *   full reassembled body (if resource_handler did not signal to delete chunks) or
 *   with empty body (if resource_handler did signal to delete chunks).
 ...
 *
 * `op_and_flags` specifies frame's val, one of:
 *
...
/*
 * File upload resource_handler.
 * This resource_handler can be used to implement file uploads with minimum code.
 * This resource_handler will process MG_EV_HTTP_PART_* events and store file data into
 * a local file.
 ...
 * Then, dispatches the request to the correct resource_handler method.
...
/*
 * Attach built-in MQTT event resource_handler to the given connection.
 *
 * The user-defined event resource_handler will receive following extra events:
 *
 * - MG_EV_MQTT_CONNACK
 * - MG_EV_MQTT_PUBLISH
 * - MG_EV_MQTT_PUBACK
 ...
 * resource_handler to store user data before `mg_mqtt_broker` creates the session.
 *
 ...
 * record val, and stored in the IO buffer. The encoded values might contain
...
/*
 * Attach built-in DNS event resource_handler to the given listening connection.
 *
 * DNS event resource_handler parses incoming UDP packets, treating them as DNS
 * requests. If incoming packet gets successfully parsed by the DNS event
 * resource_handler, a user event resource_handler will receive `MG_DNS_REQUEST` event, with
...
/* Set CoAP protocol resource_handler - trigger CoAP specific events */
